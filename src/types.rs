...
impl<Val> ValueWithExpiry<Val>
where Val: Serialize + DeserializeOwned + Debug
{
  ...
  pub fn serialize_for_storage(&self) -> Vec<u8> {
    let mut final_serialized_value =
      Vec::with_capacity(Self::TIMESTAMP_SIZE + self.raw_value.len());
...
impl<Val> ValueWithExpiry<Val>
where Val: DeserializeOwned + Debug
{
  ...
  pub fn from_slice(bytes_with_ts: &[u8]) -> Result<Self, crate::error::StoreError> {
    ...
    let raw_value = Self::raw_data_ref(bytes_with_ts).map_err(|e| {
      crate::error::StoreError::Deserialization(format!("Failed to get raw value slice: {}", e))
    })?;
...
pub enum MergeValueOperator {
  // Define specific operators as needed by application logic
  // Example:
  Append = 1,
  Increment = 2,
  SetUnion = 3,
  // REMOVE = 2, // Remove might be better handled by Delete/WriteBatch
  // UNION = 3,
  // INTERSECT = 4,
}
